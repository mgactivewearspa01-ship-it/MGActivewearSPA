document.addEventListener('DOMContentLoaded', () => {
  ...
  const API_STORAGE_KEY = 'mg_api_base';
  ...
  function resolveApiBase(){
    ...
    const fromDataset = rawDataset && !rawDataset.includes('{{') && !rawDataset.includes('}}')
      ? rawDataset
      : '';
    if(fromDataset){
    ...
    const stored = (localStorage.getItem(API_STORAGE_KEY) || '').trim();
    if(stored){
  ...
  function getCartTotals(){
    ...
    const discount = subtotal * Math.min(Math.max(discountRate, 0), 1);
      ...
      cartItems.innerHTML += `
        <div class="cart-line" style="display:flex;justify-content:space-between;gap:8px;margin-bottom:10px;">
          <div style="display:flex;gap:8px;align-items:flex-start;max-width:70%;">
            ...
            <span>${item.name}${item.size ? ' - Talla: '+item.size : ''}${item.color ? ' - Color: '+item.color : ''}<br>x${item.quantity} · $${item.price.toFixed(2)} c/u · <strong>$${lineTotal.toFixed(2)}</strong></span>
          </div>
  ...
  window.addToCart = function(name, price, size = null, color = null, image = null){
    ...
    if(existingIndex >= 0){
      ...
      cart.push({
        ...
        size,
        color,
        image: image || resolveProductImage(name),
        quantity: 1
      });
    }
  ...
  window.filterProducts = applyProductVisibility;
  window.searchProducts = applyProductVisibility;
  ...
  window.applyCoupon = function(){
    ...
    discountRate = rules[code] || 0;

    if(msg){
      msg.textContent = discountRate > 0
        ? `Cupón aplicado: ${(discountRate * 100).toFixed(0)}%`
        : (code ? 'Cupón inválido.' : '');
    }
  ...
  window.placeOrder = function(){
    if(cart.length === 0){
      const msg = document.getElementById('checkoutMessage');
      if(msg){
        msg.textContent = 'Tu canasta está vacía.';
      }
    ...
    if(!name || !phone || !paymentMethod){
      if(msg){
        msg.textContent = 'Completa nombre, teléfono y método de pago.';
      }
    ...
    const apiUrl = getApiUrl('/api/pedido');
    if(!apiUrl){
      if(msg){
        msg.textContent = 'No se puede enviar el pedido desde un archivo local. Abre la página desde el servidor (http://localhost:3000), define data-api-base en el <body> o usa ?apiBase=http://localhost:3000.';
      }
      return;
    }
    ...
    .catch((err) => {
      if(msg){
        msg.textContent = err && err.message
          ? err.message
          : 'No se pudo enviar el pedido al servidor. Verifica que el backend esté activo en el puerto 3000.';
      }
    });
  };
  ...
  window.changeImage = function(el, index){
    const card = el.closest('.card');
    const images = card ? card.querySelectorAll('.product-images img') : [];
  ...
  if(satisfactionForm){
    const messageEl = document.getElementById('satisfactionMessage');
    satisfactionForm.addEventListener('submit', async function(e){
      e.preventDefault();
      if(messageEl) messageEl.textContent = '';

      const name = document.getElementById('satisfactionName')?.value.trim() || '';
      const rating = document.getElementById('satisfactionRating')?.value || '';
      const highlight = document.getElementById('satisfactionHighlight')?.value || '';
      const comment = document.getElementById('satisfactionComment')?.value.trim() || '';

      if(!rating){
        if(messageEl) messageEl.textContent = 'Selecciona una calificación.';
        return;
      }

      const payload = { name, rating, highlight, comment };
      const submitBtn = satisfactionForm.querySelector('button[type="submit"]');
      if(submitBtn) submitBtn.disabled = true;

      try{
        const feedbackUrl = getApiUrl('/api/feedback');
        if(!feedbackUrl){
          throw new Error('No se puede enviar desde un archivo local.');
        }
        ...
        if(messageEl) messageEl.textContent = '¡Gracias por tu opinión!';
      } catch(_err){
        if(messageEl) messageEl.textContent = 'No se pudo enviar. Intenta más tarde.';
      } finally {
